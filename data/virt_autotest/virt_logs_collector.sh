...
#Typical usage: collect_support_report logs_folder host or collect_support_report logs_folder guest guest_ip guest_domain_name
function collect_support_report() {
	local logs_folder=$1
	local target_type=$2
...
echo -e "collect_support_report ${virt_logs_folder} host" | tee -a ${virt_logs_collecor_log}
collect_support_report ${virt_logs_folder} host | tee -a ${virt_logs_collecor_log}
   ...
   for guest_current in ${guest_domains_array[@]};do
       if [[ ${virt_guests_wanted} == "all" ]] || [[ ${virt_guests_wanted} =~ .*${guest_current}.* ]];then
          if [[ ${guests_inactive_array[@]} =~ .*${guest_current}.* ]];then 
             ...
             echo -e "collect_support_report ${virt_logs_folder} guest ${guest_hash_ipaddr[${guest_hash_index}]} ${guest_current}" | tee -a ${virt_logs_collecor_log}
             collect_support_report ${virt_logs_folder} guest ${guest_hash_ipaddr[${guest_hash_index}]} ${guest_current} | tee -a ${virt_logs_collecor_log}
             virt_logs_collector_result=$(( ${virt_logs_collector_result} | $? ))
