...
sub register_guest_name {
    ...
    $args{usedns} //= 0;
    $args{domainname} //= '';
    croak("Guest and ip address for must be given") if (!$args{guest} or !$args{ipaddr});
    ...
    foreach (keys %guest_matrix) {
        ...
        if ($args{usedns}) {
            if (script_run("timeout --kill-after=1 --signal=9 15 ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i $args{keyfile} root\@$guest_matrix{$_}{ipaddr} ls") == 0) {
                $temp = assert_script_run("timeout --kill-after=1 --signal=9 15 ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i $args{keyfile} root\@$guest_matrix{$_}{ipaddr} \"echo -e $_.$guest_matrix{$_}{domainname} > /etc/hostname\;hostnamectl hostname $_.$guest_matrix{$_}{domainname};sync\"");
            }
            else {
                enter_cmd("clear", wait_still_screen => 3);
                enter_cmd("timeout --kill-after=1 --signal=9 30 ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i $args{keyfile} root\@$guest_matrix{$_}{ipaddr} \"echo -e $guest_matrix{$_}{domainname} > /etc/hostname\;hostnamectl hostname $guest_matrix{$_}{domainname};sync\"", wait_still_screen => 3);
