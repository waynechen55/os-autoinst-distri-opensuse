    ...
    foreach my $_guest (split(/ /, $args{_guest})) {
        ...
        if (get_var('DOWNLOAD_GUEST_ASSETS')) {
            if (script_output("curl --silent -I $_guest_disk_url | grep -E \"^HTTP\" | awk -F \" \" \'{print \$2}\'") != "200") {
                ...
                script_run("curl $_guest_disk_url -o $args{_assetdir}/$_guest_asset_name.disk");
            }
            if (script_output("curl --silent -I $_guest_config_url | grep -E \"^HTTP\" | awk -F \" \" \'{print \$2}\'") != "200") {
                ...
                script_run("curl $_guest_config_url -o $args{_assetdir}/$_guest_asset_name.xml");
            }
        }
...
sub wait_guest {
    ...
    foreach my $_guest (split(/ /, $args{_guest})) {
        ...
        add_guest_to_hosts($_guest, $_guest_matrix{$_guest}{ipaddr});
        save_screenshot;
...
");
            ...
            if ($_guest_matrix{$_guest}{netname} ne 'br0') {
                ...
                if ($args{_usedns} == 1) {
                    ...
                    $_dnsmasq_command = "/usr/sbin/dnsmasq --bind-dynamic --listen-address=$_ipaddr --dhcp-range=$_startaddr,$_endaddr,$_netmask,8h --interface=br123 --dhcp-authoritative --no-negcache --dhcp-option=option:router,$_ipaddr --log-queries --log-dhcp --dhcp-sequential-ip --dhcp-client-update --domain=$_guest_params{dns_domainname} --log-dhcp --dhcp-fqdn --dhcp-sequential-ip --dhcp-client-update --domain-needed --dns-loop-detect --server=/$_guest_params{dns_domainname}/$_ipaddr --server=/$_ipaddr_rev/$_ipaddr --no-daemon";
                }
                else {
                    $_dnsmasq_command = "/usr/sbin/dnsmasq --bind-dynamic --listen-address=$_ipaddr --dhcp-range=$_startaddr,$_endaddr,$_netmask,8h --interface=br123 --dhcp-authoritative --no-negcache --dhcp-option=option:router,$_ipaddr --log-queries --log-dhcp --dhcp-sequential-ip --dhcp-client-update --no-daemon";
                }
                my $_dnsmasq_command_query = "/usr/sbin/dnsmasq --bind-dynamic --listen-address=$_ipaddr.*--dhcp-range=$_startaddr,$_endaddr,$_netmask,8h --interface=br123 --dhcp-authoritative --no-negcache --dhcp-option=option:router,$_ipaddr.*--no-daemon";
                ...
                if ($_temp == 0 and $args{_usedns} == 1) {
                    ...
                    $_detect_signature = script_output("cat /etc/resolv.conf | grep \"#Modified by parallel_guest_migration_base.pm module\"", proceed_on_failure => 1);
...
sub test_guest_network {
    ...
    foreach my $_guest (split(/ /, $args{_guest})) {
        ...
        $_temp = script_run("timeout --kill-after=1 --signal=9 20 ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i $args{_keyfile} root\@$_guest ping -c5 $_ping_target");
        $_temp |= script_run("timeout --kill-after=1 --signal=9 20 ping -c5 $_guest_matrix{$_guest}{ipaddr}");
        $_temp |= 1 if (($_guest_matrix{$_guest}{netmode} eq 'host') and (check_port_state($_guest_matrix{$_guest}{ipaddr}, 22, 3) == 0));
        $_ret |= $_temp;
        save_screenshot;
