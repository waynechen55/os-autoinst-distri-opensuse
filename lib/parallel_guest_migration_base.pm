...
sub config_ssh_pubkey_auth {
    ...
    setup_common_ssh_config(identity_file => $args{_keyfile});
...
sub config_host_shared_storage {
    ...
    if ($args{_type} eq 'nfs') {
        if ($_host_params{external_nfsshare}) {
            ...
            #assert_script_run("mount -t nfs $_temppath $args{_mntpath}");
...
sub start_guest {
    ...
    foreach my $_guest (split(/ /, $args{_guest})) {
        ...
        if ($_temp == 0 and $args{_wait} == 1) {
            $_temp |= $self->wait_guest(_guest => $_guest);
...
addresses and domain names are supported as strings separated by space. Other arguments
include _keyfile (key file for passwordless ssh connection) and _usedns (0 or 1).
Call croak to die if subroutine fails and argument _die is set. 
=cut
...
sub catalogue_guest {
    ...
    croak("Cataloguing failed for certain guest") if ($_ret != 0 and $args{_die} == 1);
    return $_ret;
}

=head2 systemctl_guest

Call manage_guest_service to manage service/target/socket unit in guest. Multiple
guests and corresponding ip addresses are supported as strings separated by space.
Other arguments include _keyfile (key file for passwordless ssh connection), 
...
sub systemctl_guest {
    ...
    croak("Systemctl $args{_operation} failed for certain guest") if ($_ret != 0 and $args{_die} == 1);
    return $_ret;
}
...
sub wait_guest {
    ...
    foreach my $_guest (split(/ /, $args{_guest})) {
        ...
        $_temp |= $self->systemctl_guest(_guest => $_guest, _operation => 'restart', _unit => 'network.service') if ($_temp == 0 and $args{_usedns});
        $_ret |= $_temp;
        save_screenshot;
        record_info("Guest $_guest waiting failed", "Failed to wait guest up and running", result => 'fail') if ($_temp != 0);
    }
    croak("Waiting for guest up and running failed for certain guest") if ($_ret != 0 and $args{_die} == 1);
    return $_ret;
}
...
sub wait_guest_ssh {
    ...
    foreach my $_guest (split(/ /, $args{_guest})) {
        my $_temp = 1;
        $_temp = script_retry("nc -zvD $_guest 22", option => '--kill-after=1 --signal=9', delay => 1, retry => $args{_retry}, die => 0);
        save_screenshot;
        $_ret |= $_temp;
        record_info("Guest $_guest ssh failed", "Failed to detect open port 22 on guest $_guest", result => 'fail') if ($_temp != 0);
    }
    croak("ssh connection failed for certain guest") if ($_ret != 0 and $args{_die} == 1);
    return $_ret;
}
...
");
            ...
            if ($_guest_matrix{$_guest}{netname} ne 'br0') {
                if (script_run("ip route show all | grep \"$_guest_matrix{$_guest}{netname} \"") != 0) {
                    script_run("ip -d addr del $_ipaddr/$_masklen dev $_guest_matrix{$_guest}{netname}; ip -d link set dev $_guest_matrix{$_guest}{netname} down; ip -d link del dev $_guest_matrix{$_guest}{netname}");
                    $_temp = script_retry("ip -d link add $_guest_matrix{$_guest}{netname} type $_guest_matrix{$_guest}{nettype}; ip -d addr flush dev $_guest_matrix{$_guest}{netname}", option => '--kill-after=1 --signal=9', retry => 3, die => 0);
                    $_temp |= script_retry("ip -d addr add $_ipaddr/$_masklen dev $_guest_matrix{$_guest}{netname} && ip -d link set $_guest_matrix{$_guest}{netname} up", option => '--kill-after=1 --signal=9', retry => 3, die => 0);
                    $_temp |= script_run("iptables --append FORWARD --in-interface $_guest_matrix{$_guest}{netname} -j ACCEPT");
                    my $_find_netdev = grep(/^$_guest_matrix{$_guest}{netname}$/, split(/\n/, script_output("ip route show | grep -v default | awk \'{print \$3}\'", type_command => 1, proceed_on_failure => 1)));
                    $_temp |= 1 && record_info("Network $_guest_matrix{$_guest}{netname} creation failed", script_output("ip addr show all;ip route show all", type_command => 1, proceed_on_failure => 1), result => 'fail') if (!$_find_netdev);
                }
                else {
                    $_temp = 0;
                }
                my ($_no_use, $_ipaddr_rev) = virt_autotest::utils::parse_subnet_address_ipv4("$_ipaddr/$_masklen");
                my $_dnsmasq_command = "/usr/sbin/dnsmasq --bind-dynamic --listen-address=$_ipaddr --dhcp-range=$_startaddr,$_endaddr,$_netmask,8h --interface=$_guest_matrix{$_guest}{netname} --dhcp-authoritative --no-negcache --dhcp-option=option:router,$_ipaddr --log-queries --log-dhcp --dhcp-sequential-ip --dhcp-client-update --domain=$_guest_params{dns_domainname} --local=/$_guest_params{dns_domainname}/ --address=/$_guest_params{dns_domainname}/$_ipaddr --log-dhcp --dhcp-fqdn --dhcp-sequential-ip --dhcp-client-update --domain-needed --dns-loop-detect --server=/$_guest_params{dns_domainname}/$_ipaddr --server=/$_ipaddr_rev/$_ipaddr --no-daemon";
                my $_dnsmasq_command_query = "/usr/sbin/dnsmasq --bind-dynamic --listen-address=$_ipaddr.*--dhcp-range=$_startaddr,$_endaddr,$_netmask,8h --interface=$_guest_matrix{$_guest}{netname} --dhcp-authoritative --no-negcache --dhcp-option=option:router,$_ipaddr.*--no-daemon";
                if (!script_output("ps ax | grep -i -E \"$_dnsmasq_command_query\" | grep -v grep | awk \'{print \$1}\'", type_command => 1, proceed_on_failure => 1)) {
                    ...
                    $_temp |= 1 && record_info("DHCP service failed on $_guest_matrix{$_guest}{netname}", "Command to start DHCP service is $_dnsmasq_command", result => 'fail') if (!$_find_dnsmasq);
                }
                if ($_temp == 0 and $args{_usedns} == 1) {
...
");
                    ...
                    my $_detect_name_server = script_output("cat /etc/resolv.conf | grep \"nameserver $_ipaddr\"", proceed_on_failure => 1);
                    ...
                    assert_script_run("awk -v dnsvar=$_ipaddr \'done != 1 && /^nameserver.*\$/ { print \"nameserver \"dnsvar\"\"; done=1 } 1\' /etc/resolv.conf > /etc/resolv.conf.tmp") if ($_detect_name_server eq '');
                    ...
                    if (script_run("ls /etc/resolv.conf.tmp") == 0) {
                        ...
                        assert_script_run("echo \'#Modified by parallel_guest_migration_base module network $_ipaddr\' >> /etc/resolv.conf");
                    }
                ...
                (my $_host_default_network_connection = script_output("nmcli -f NAME,DEVICE connection show | $_host_default_network_interface", proceed_on_failure => 0)) =~ s/\s*$_host_default_network_interface\s*$//;
...
");
                    ...
                    $_temp = script_retry("nmcli connection load $_nmconnection_config_file", retry => 3, die => 0);
...
");
                    ...
                    script_run("chmod 700 $_nmconnection_config_file");
                    $_temp |= script_retry("nmcli connection load $_nmconnection_config_file", retry => 3, die => 0);
                    $_temp |= script_retry("nmcli connection up br0", timeout => 60, delay => 15, retry => 3, die => 0);
                    $_temp |= script_retry("nmcli connection up $_host_default_network_interface", timeout => 60, delay => 15, retry => 3, die => 0);
