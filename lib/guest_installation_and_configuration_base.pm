...
our %guest_params = (
    ...
    'guest_registration_server'   => '',    #This is the address of registration/subscription server of content provider. It can be left empty if not needed.
    'guest_registration_username' => '', #This is the username to be used in registration/subscription. It can be email address, free text or empty if not needed.
    'guest_registration_password' => '', #This is the password to be used in registration/subscription. It is normally used together with [guest_registration_username] or empty if not needed.
    'guest_registration_code' => '',     #This is the code or key to used in registraiton/subscription.
                                         #It can be used together with [guest_registration_username], standalone or empty if not needed.
    'guest_registration_extensions' => '', #This refers to additional modules/extensions/products to be registered together with guest os or by using individual registration/subscription code/key.
                                           #Multiple modules/extensions/products are separated by hash key. For example, "sle-module-legacy#sle-module-basesystem#SLES-LTSS".
                                           #Can be left empty if not needed.
    'guest_registration_extensions_codes' => '', #This refers to registration/subscription codes/keys to be used by modules/extensions/products in [guest_registration_extensions].
     #Can be empty if not needed, but if anyone in [guest_registration_extensions] needs its own code/key, all the others should also be given theirs even empty.
     #For example, "sle-module-legacy#sle-module-basesystem#SLES-LTSS" needs "##SLES-LTSS-CODE-OR-KEY" to be set in [guest_registration_extensions_codes].
     #And the codes/keys should be separated by hash key and put in the same order as their corresponding modules/extensions/products in [guest_registration_extensions].
...
sub config_guest_installation_automation_registration {
    ...
    if ($self->{guest_do_registration} eq 'false') {
        ...
        assert_script_run("sed -ri \'s/##Registration-Server##/$self->{guest_registration_server}/g;\' $self->{guest_installation_automation_file}") if ($self->{guest_registration_server} ne '');
        assert_script_run("sed -ri \'s/##Registration-UserName##/$self->{guest_registration_username}/g;\' $self->{guest_installation_automation_file}") if ($self->{guest_registration_username} ne '');
        assert_script_run("sed -ri \'s/##Registration-Password##/$self->{guest_registration_password}/g;\' $self->{guest_installation_automation_file}") if ($self->{guest_registration_password} ne '');
        assert_script_run("sed -ri \'s/##Registration-Code##/$self->{guest_registration_code}/g;\' $self->{guest_installation_automation_file}") if ($self->{guest_registration_code} ne '');
