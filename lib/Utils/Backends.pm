...
using FQDN. Record failure and even terminate test run if "netconfig update -f" 
can not help. Arguments are address of the system on which netconfig will operate
...
sub update_ssh_console_netconfig {
    ...
    script_run("$sshcmd cp /etc/resolv.conf /etc/resolv_backup.conf");
    script_run("$sshcmd sed -i -r \'/^nameserver.*\$/d\' /etc/resolv.conf");
    ...
    if ($pingret != 0) {
        ...
        script_run("$sshcmd mv /etc/resolv_backup.conf /etc/resolv.conf");
