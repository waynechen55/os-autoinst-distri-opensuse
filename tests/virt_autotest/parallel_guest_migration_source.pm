...
sub run_test {
    ...
    $self->do_peer_initialization(_keyfile => $parallel_guest_migration_base::_host_params{ssh_keyfile});
    barrier_wait('PEER_INITIALIZATION_DONE');

    $self->prepare_host(keyfile => $parallel_guest_migration_base::_host_params{ssh_keyfile});
...
sub prepare_guest {
    ...
    $self->config_ssh_pubkey_auth(_addr => $guest, _keyfile => $parallel_guest_migration_base::_guest_params{ssh_keyfile}, _overwrite => 0, _host => 0);
...
sub guest_migration_test {
    ...
    foreach my $guest (keys %parallel_guest_migration_base::_guest_matrix) {
        while (my ($testindex, $test) = each(@guest_migration_test)) {
            ...
            $counter += 1;
            $test_stop_time = time();
            $parallel_guest_migration_::test_result{$guest}{$command}{test_time} = strftime("\%H:\%M:\%S", gmtime($test_stop_time - $test_start_time));
