    ...
    if (is_kvm_host) {
        foreach (keys %virt_autotest::common::guests) {
            if (is_sle('>=15') and ($_ =~ /12-sp5/img)) {
                record_info("PMSUSPEND to hyrbrid is not supported here", "Guest $_ on kvm sles 15-sp4 host");
                next;
            }
        ...
        foreach (keys %virt_autotest::common::guests) {
            if (is_sle('>=15') and ($_ =~ /12-sp5/img)) {
                record_info("PMSUSPEND to disk is not supported here", "Guest $_ on kvm sles 15-sp4 host");
                next;
            }
