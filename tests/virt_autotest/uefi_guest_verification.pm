...
sub check_guest_pmsuspend_enabled {
    ...
    if (is_kvm_host) {
        foreach (keys %virt_autotest::common::guests) {
            if (is_sle('=15-sp4') and ($_ =~ /12-sp5/img)) { 
                record_soft_failure("PMSUSPEND implementation for kvm fullvirt uefi virtual machine is incomplete.", "Suspend to hybrid is not supported."); 
	        next;
	    }
        ...
        foreach (keys %virt_autotest::common::guests) {
            if (is_sle('=15-sp4') and ($_ =~ /12-sp5/img)) { 
                record_soft_failure("PMSUSPEND implementation for kvm fullvirt uefi virtual machine is incomplete.", "Suspend to disk is not supported.");
                next;
            }
